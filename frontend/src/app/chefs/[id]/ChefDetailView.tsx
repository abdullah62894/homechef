...
import { foodHref } from "@/lib/slugs";
  ...
  return (
    <section className="mx-auto max-w-5xl px-4 py-12 sm:py-16">
      <nav aria-label="Breadcrumb" className="text-sm text-gray-500">
        ...
        <Link href="/chefs" className="hover:underline">Chefs</Link>
        <span className="mx-1.5">/</span>
        <span className="text-gray-900">{chef.displayName}</span>
      </nav>
      ...
      <div className="mt-12">
        ...
        {foods.length === 0 ? (
          ...
          <>
            {/* Today's Menu */}
            {foods.some((f) => f.isAvailable) && (
              <div className="mt-6">
                <div className="flex items-center gap-2 mb-4">
                  <span className="inline-flex items-center gap-1.5 rounded-full bg-green-100 px-3 py-1 text-xs font-semibold text-green-700 ring-1 ring-green-200">
              ...
              <div className="mt-8">
                ...
                <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                  {foods.filter((f) => !f.isAvailable).map((food) => (
                    <div
                      key={food.id}
                      className="flex flex-col justify-between rounded-xl border border-gray-200 bg-gray-50/50 p-5 opacity-70"
                    >
                      <div>
                        <div className="flex items-center justify-between gap-2">
                          <div className="flex flex-wrap gap-1">
                            {food.cuisineNames?.length > 0 ? (
                              food.cuisineNames.map((name) => (
                                <span key={name} className="rounded bg-gray-200 px-2 py-0.5 text-xs font-medium text-gray-600">
                                  {name}
                                </span>
                              ))
                            ) : (
                          ...
                          <span className="rounded-full px-2 py-0.5 text-[11px] font-semibold bg-gray-100 text-gray-500">
                            Sold out
                          </span>
                        </div>
                        ...
                        <p className="mt-1 text-sm text-gray-500 line-clamp-2">{food.description}</p>
                      </div>
